...
| 2 | Partial mobile/account number, caller stops early | Buffer holds partial state indefinitely; never marks complete below expected length | `test_number_buffer_never_completes_early` |
...
| 6 | Wrong-length / impossible number reaches a tool call | `verify_customer`/`send_otp`/`verify_otp` refuse with `invalid_number_format` before hitting the backend | gate added in `tools/registry.py::_validate_number_args` (exercised indirectly; see Manual QA #16 for the live-call version) |
...
| 10 | Background word during an active topic (TV, stray word) | Active topic (e.g. internet fault) does NOT flip on a single unrelated word | `test_topic_stability_ignores_single_stray_background_word` |
...
| 17 | Wrong verification (account number resolves to someone else, or lookup fails) | `verify_customer` gate already refuses writes without fresh verification (`tools/registry.py`); confirm the LLM's spoken response on a failed lookup is graceful and re-prompts for a corrected number rather than looping silently. |
