...
export function useTicket(id: string) {
  return useQuery({ queryKey: ["ticket", id], queryFn: () => repositories.tickets.get(id), enabled: !!id });
}

export function useCustomers(q?: string) {
  return useQuery({ queryKey: ["customers", q], queryFn: () => repositories.customers.list(q) });
}
...
export function useEscalations() {
  return useQuery({ queryKey: ["escalations"], queryFn: () => repositories.escalations.list() });
}

export function useConversations(q?: string) {
  return useQuery({ queryKey: ["conversations", q], queryFn: () => repositories.conversations.list(q) });
}
