...
export default function ExecutivesPage() {
  ...
  return (
    <>
      <PageHeader title="Executive Panel" description="Support team availability and performance." />
      {!isLoading && rows.length === 0 ? (
        <EmptyState
          icon={<BadgeCheck className="h-6 w-6" />}
          title="No executive data yet"
          description="Connect your ACD/CRM or executive roster to populate live availability, workload and performance here."
        />
      ) : (
              ...
              <Card key={e.id}>
                <CardContent className="p-5">
                  ...
                  <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                    <div className="rounded-lg bg-muted/40 py-2"><p className="text-lg font-bold">{e.calls_today}</p><p className="text-[10px] uppercase text-muted-foreground">Calls</p></div>
                    <div className="rounded-lg bg-muted/40 py-2"><p className="text-lg font-bold">{e.avg_resolution_min || "—"}<span className="text-xs">m</span></p><p className="text-[10px] uppercase text-muted-foreground">Avg Res</p></div>
                    <div className="rounded-lg bg-muted/40 py-2"><p className="text-lg font-bold">{e.satisfaction || "—"}</p><p className="text-[10px] uppercase text-muted-foreground">CSAT</p></div>
                  </div>
                </CardContent>
              </Card>
            );
